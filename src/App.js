...
// ── HOME VIEW ─────────────────────────────────────────────────────────────────
const HomeView = ({ projects, setProjects, municipios, setView }) => {
...
export default function App() {
  ...
  const [projects,   setProjects]   = useState(() => {
    ...
    return saved ? JSON.parse(saved) : PROJECTS_INIT;
  });
  ...
  return (
    <div style={{ display: "flex", height: "100vh", background: "#0d1624", fontFamily: "'DM Sans','Inter',system-ui,sans-serif", overflow: "hidden" }}>
      ...
      {view === "home"      && <HomeView      projects={projects} setProjects={setProjects} municipios={municipios} setView={setView} />}
